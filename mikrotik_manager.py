        ...
        try:
            ...
            try:
                if total_users <= 1 or temp_users == 0:
                    ...
                    _db.execute_query(
                        "UPDATE credential_requests SET status='expired' WHERE status='active' AND expires_at <= NOW() AND wan_ip=%s",
                        (ip_address,)
                    )
                    rows = _db.execute_query(
                        "SELECT COUNT(*) as cnt FROM credential_requests WHERE wan_ip=%s AND status='active'",
                        (ip_address,)
                    ) or [{"cnt": 0}]
